...
contract JobManager is ReentrancyGuard {
    ...
    event JobPosted(uint256 indexed jobId, address indexed requester, uint256 rewardUsd, uint256 rewardEth);
     ...
     */
    function postJob(string memory dataUrl, uint256 rewardUsd) external payable nonReentrant {
        ...
        s_jobCounter++;
        emit JobPosted(jobId, msg.sender, rewardUsd, msg.value);
